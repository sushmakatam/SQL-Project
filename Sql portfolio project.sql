...
select State, round(avg(Sex_ratio),0) as avgsexratio, avg(Growth)*100 from data1 group by State order by avgsexratio desc;
...
select distinct state from data1 where lower(state) like 'a%' and lower(state) like '%h' ;
...
(select a.district, a.state, a.literacy/100 as literacy_ratio,b.population from data1 a inner join Data2 b 
